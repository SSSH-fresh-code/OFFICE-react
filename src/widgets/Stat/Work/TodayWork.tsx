...
export function TodayWork({ refetch, work = [], isPending, isSuccess }: TodayWorkProps) {
  ...
  >
    <div id="goToWork" className="rounded-lg border bg-card text-card-foreground shadow-sm" data-v0-t="card">
      ...
      {
        (isSuccess && work[0] && !work[0].offTime) && (
          <TodayGoToWork subtitle="업무를 기록하거나 퇴근할 수 있어요.">
            <textarea
              ...
              defaultValue={work[0].workDetail}
            ></textarea>
      ...
      }
      {
        (isSuccess && work[0] && work[0].offTime) && (
          <TodayGoToWork subtitle="오늘 업무는 이미 마쳤어요.">
            <div className="col-span-1 px-4 py-8 flex flex-col gap-3 bg-blue-500 rounded-md font-extrabold text-white text-lg items-center">
              <span><WorkCheckIcon /></span>
              <span>고생하셨어요!</span>
            </div>
          </TodayGoToWork>
